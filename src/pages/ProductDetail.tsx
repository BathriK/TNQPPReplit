...
const ProductDetail = () => {
  ...
  }, [productId, selectedMonth, selectedYear]);
          ...
          <>
            ...
            <Tabs value={activeTab} className="w-full" onValueChange={setActiveTab}>
              ...
              <TabsContent value="roadmap" className="tab-content">
                <div className="flex justify-between items-center grid-margin-3">
                  ...
                  <div className="flex items-center grid-gap-4">
                    ...
                    <VersionHistory 
                      ...
                      onCreateNew={() => navigate(`/products/${productId}/edit?tab=roadmap`)} 
                    />
                  </div>
                </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {roadmapVersions.length > 0 ? (
                        roadmapVersions.sort((a, b) => parseFloat(b.version) - parseFloat(a.version)).map(roadmap => (
                          <TableRow key={roadmap.id}>
                            <TableCell className="font-medium text-gray-900">{roadmap.version}</TableCell>
                            <TableCell className="text-gray-900">{formatDate(roadmap.createdAt)}</TableCell>
                            <TableCell>
                ...
                <div className="flex justify-between items-center grid-margin-3">
                  ...
                  <div className="flex items-center grid-gap-4">
                    ...
                    <VersionHistory 
                      ...
                      onCreateNew={() => navigate(`/products/${productId}/edit?tab=goals`)} 
                    />
                  </div>
                </div>
                ...
                {getSelectedPlan() && getSelectedPlan()?.items && getSelectedPlan()?.items.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full">
                      <thead className="bg-tnq-blue">
                        <tr>
                          <th className="text-left py-3 px-4 font-medium text-white">Feature Name</th>
                          <th className="text-left py-3 px-4 font-medium text-white">Category</th>
                          <th className="text-left py-3 px-4 font-medium text-white">Priority</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {getSelectedPlan()?.items.map((item: any) => (
                          <tr key={item.id}>
                            ...
                            <td className="py-3 px-4 text-gray-900">{item.priority || "Medium"}</td>
                            <td className="py-3 px-4 text-gray-900">{item.description}</td>
                            <td className="py-3 px-4 text-gray-900">{item.source || "Internal"}</td>
                            <td className="py-3 px-4">
                              <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
